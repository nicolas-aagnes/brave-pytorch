...
def run_optical_flow(vid_item, dev_id=0):
    ...
    cmd = osp.join(
        args.df_path, "build/extract_gpu"
...
def run_warp_optical_flow(vid_item, dev_id=0):
    ...
    cmd = osp.join(
        args.df_path + "build/extract_warp_gpu"
...
def parse_args():
    ...
    parser.add_argument("--download_dir", type=str, default="./data/hmdb51")
    parser.add_argument("--src_dir", type=str, default="./data/hmdb51/videos")
    parser.add_argument("--out_dir", type=str, default="./data/hmdb51/rawframes")
    parser.add_argument("--frame_path", type=str, default="./data/hmdb51/rawframes")
    parser.add_argument(
        "--anno_dir",
        type=str,
        default="./data/hmdb51/testTrainMulti_7030_splits",
    )
    parser.add_argument(
        "--out_list_path",
        type=str,
    ...
    parser.add_argument(
        "--flow_type", type=str, default=None, choices=[None, "tvl1", "warp_tvl1"]
    )
    parser.add_argument(
        "--df_path",
        type=str,
        default="./dense_flow",
        help="need dense flow implementation",
    )
    parser.add_argument(
        "--out_format",
        type=str,
    ...
    parser.add_argument(
        "--new_height", type=int, default=256, help="resize image height"
    )
    ...
    parser.add_argument(
        "--subset", type=str, default="train", choices=["train", "val", "test"]
    )
    parser.add_argument(
        "--format", type=str, default="rawframes", choices=["rawframes", "videos"]
    )
...
def decode_video(args):
    ...
    if args.flow_type == "tvl1":
        ...
        pool.map(
            run_warp_optical_flow, zip(fullpath_list, vid_list, range(len(vid_list)))
        )
    else:
...
def parse_hmdb51_splits(args):
    ...
    class_ind = [
        x.strip().split() for x in open(os.path.join(args.anno_dir, "classInd.txt"))
    ]
    ...
    for i in range(1, 4):
        train_list = [
            line2rec(x)
            for x in open(os.path.join(args.anno_dir, "trainlist{:02d}.txt".format(i)))
        ]
        test_list = [
            line2rec(x)
            for x in open(os.path.join(args.anno_dir, "testlist{:02d}.txt".format(i)))
        ]
...
def parse_directory(
    ...
    for i, f in enumerate(frame_folders):
        ...
        if x_cnt != y_cnt:
            raise ValueError(
                "x and y direction have different number " "of flow images. video: " + f
            )
        if i % 200 == 0:
...
def build_file_list(args):
    ...
    if args.format == "rawframes":
        ...
        if args.level == 1:
            ...
            video_list = glob.glob(osp.join(args.frame_path, "*", "*"))
        frame_info = {
            osp.relpath(x.split(".")[0], args.frame_path): (x, -1, -1)
            for x in video_list
        }
    ...
    if len(split_tp) > 1:
        for i, split in enumerate(split_tp):
            ...
            filename = "{}_train_split_{}_{}.txt".format(
                args.dataset, i + 1, args.format
            )
            with open(osp.join(out_path, filename), "w") as f:
                f.writelines(lists[0][0])
            filename = "{}_val_split_{}_{}.txt".format(args.dataset, i + 1, args.format)
            with open(osp.join(out_path, filename), "w") as f:
                f.writelines(lists[0][1])
    else:
        lists = build_split_list(split_tp[0], frame_info, shuffle=args.shuffle)
        filename = "{}_{}_list_{}.txt".format(args.dataset, args.subset, args.format)
