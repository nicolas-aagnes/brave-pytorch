...
class DCGAN(pl.LightningModule):
    ...
    def _get_discriminator(self) -> nn.Module:
        discriminator = DCGANDiscriminator(
            self.hparams.feature_maps_disc, self.hparams.image_channels
        )
    ...
    def on_train_start(self):
        self.logger.log_hyperparams(self.hparams, {"Loss/disc": 0, "Loss/gen": 0})

    def training_epoch_end(self, outputs):
        # Outputs is a list of dict output for exach optimizer from the training step.
        self.log(
            "Loss/disc",
            torch.mean(torch.stack(tuple(out["loss"] for out in outputs[0]))),
            on_epoch=True,
        )
        self.log(
            "Loss/gen",
            torch.mean(torch.stack(tuple(out["loss"] for out in outputs[1]))),
            on_epoch=True,
        )
...
def cli_main(args=None):
    ...
    parser.add_argument(
        "--dataset", default="mnist", type=str, choices=["lsun", "mnist"]
    )
    ...
    if script_args.dataset == "lsun":
        ...
        dataset = LSUN(
            root=script_args.data_dir, classes=["bedroom_train"], transform=transforms
        )
        image_channels = 3
        ...
        print("Initial dataset size of", len(dataset))
        ...
        print("Final dataset size of", len(dataset))
        image_channels = 1
    ...
    def get_module_hparams(init_method, args):
        import inspect

        hparams = inspect.getfullargspec(init_method).args
        return {
            hparam: getattr(args, hparam) for hparam in hparams if hasattr(args, hparam)
        }

    dcgan_hparams = get_module_hparams(DCGAN.__init__, args)

    model = DCGAN(**dcgan_hparams, image_channels=image_channels)
    callbacks = [
    ...
    print(trainer.log_every_n_steps, len(dataloader), "SHOULD BE EQUAL")
